...
def execute_ocrmypdf(
    *,
    ...
    log.debug(
        f"OCRmyPDF input_file={file_path} output_file={output_path} kwargs: {ocrmypdf_kwargs}"
    )
    try:
        ...
        log.info("PDF déjà OCRisé (texte détecté). Ignoré grâce à skip_text/guard.")
        return
...
class HandleObserverEvent(PatternMatchingEventHandler):
    ...
    def on_any_event(self, event):
        ...
        arch = Path(self._settings["archive_dir"])
        inp = Path(self._settings["input_dir"])

        log.debug("Event %s on %s", event.event_type, p)

        # Ignorer les fichiers déjà OCRisés (ex: *_ocr.pdf)
        if p.name.lower().endswith("_ocr.pdf"):
            return

        # Skip événements dans output/archive, sauf si identiques à input
        try:
            is_in_out = p.is_relative_to(out)
            is_in_arch = p.is_relative_to(arch)
            ...
            def _under(child: Path, parent: Path) -> bool:
                try:
                    ...
                    return False
            is_in_out = _under(p, out)
            is_in_arch = _under(p, arch)

        if out.resolve() != inp.resolve() and is_in_out:
            return
        if arch.resolve() != inp.resolve() and is_in_arch:
            return
        ...
        if event.event_type in ["created", "moved"]:
            execute_ocrmypdf(
                ...
                ocrmypdf_kwargs=self._settings["ocrmypdf_kwargs"],
                on_success_delete=self._settings["on_success_delete"],
                on_success_archive=self._settings["on_success_archive"],
                poll_new_file_seconds=self._settings["poll_new_file_seconds"],
                retries_loading_file=self._settings["retries_loading_file"],
                output_dir_year_month=self._settings["output_dir_year_month"],
            )
...
def main(
    ...
    output_dir: Annotated[
        ...
        typer.Option(
            ...
            writable=True,
            resolve_path=True,
            help="Directory to write OCR’d PDFs",
        ),
    ] = Path("/output"),
    archive_dir: Annotated[
        ...
        typer.Option(
            ...
            writable=True,
            resolve_path=True,
            help="Directory to move originals after OCR (if archiving)",
        ),
    ] = Path("/processed"),
    ...
    # Honor env vars if flags weren’t provided (fallback when still on defaults)
    env_in = os.getenv("OCR_INPUT_DIRECTORY")
    env_out = os.getenv("OCR_OUTPUT_DIRECTORY")
    env_arch = os.getenv("OCR_ARCHIVE_DIRECTORY")

    if env_in and str(input_dir) == "/input":
        input_dir = Path(env_in).expanduser().resolve()
    if env_out and str(output_dir) == "/output":
        output_dir = Path(env_out).expanduser().resolve()
    if env_arch and str(archive_dir) == "/processed":
        archive_dir = Path(env_arch).expanduser().resolve()

    # Ensure directories exist (after overrides)
    if not input_dir.exists():
        raise FileNotFoundError(f"Input directory does not exist: {input_dir}")
    output_dir.mkdir(parents=True, exist_ok=True)
    if on_success_archive:
        archive_dir.mkdir(parents=True, exist_ok=True)

    # Auto-switch to polling on Windows-mounted paths
    if not use_polling and str(input_dir).startswith("/mnt/"):
        log.info("Input is on /mnt (Windows FS); switching to polling for reliability.")
        use_polling = True

    # Load JSON settings (string or file path)
    ...
    if "input_file" in json_settings or "output_file" in json_settings:
        log.error("OCR_JSON_SETTINGS (--ocr-json-settings) may not specify input/output file")
        sys.exit(1)

    # Appliquer valeurs par défaut sûres
    effective_kwargs = {**json_settings, "deskew": deskew}
    # N’activer skip_text que s’il n’est pas explicitement défini par l’utilisateur
    effective_kwargs.setdefault("skip_text", True)
    ...
    handler = HandleObserverEvent(
        ...
        settings={
            ...
            "ocrmypdf_kwargs": effective_kwargs,
