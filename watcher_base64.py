...
def ocr_to_bytes(pdf_path: Path) -> Tuple[bytes, bool]:
...
class PdfToBase64Handler(FileSystemEventHandler):
    ...
    def _handle_path(self, path: Path) -> None:
        ...
        try:
            pdf_bytes, used_original = ocr_to_bytes(path)
...
def main(
    ...
    handler = PdfToBase64Handler(input_dir, output_dir, use_polling, retries)
